...
function runDeliverable1(params, format)
    ...
    % Use the same effective mass and AMB parameters as in the step-response
    % model, and the 4‑DOF rigid-body notes for radial and tilting modes.
    % AMB separation (lever arm l in the notes):
    %   l = axial_clear + h_fly + axial_clear + h_motor + axial_clear
    L_amb = 3 * axial_clear + h_fly + h_motor;
    dynStiff = computeDynamicStiffness(p, pc, ambParams, m_rotating, J, L_amb);
    ...
    %
    % From the notes, imbalance force magnitude is
    %   F_d(Ω) = m * e * Ω^2
    % and steady‑state runout amplitude is
    %   r(Ω) = |F_d(Ω)| / |K_dyn(jΩ)|
    % Here we use the radial dynamic stiffness from dynStiff.

    runout = computeRunoutVsSoc(soc_vec, omega_vec, dynStiff, m_rotating, p);
...
function [losses, T_rot_C, omega_vec] = computeLossesAndTemperatureVsSoc( ...
    ...
    % Shaft / flywheel geometry for radiating area
    r_shaft = d_shaft / 2;
    axial_clear = p.clearance.axial_m;
    ambParams = ambParameters(d_shaft, p.amb.ratedForce_N);
    h_shaft = (5 * axial_clear) + h_fly + L_motor + 2 * ambParams.axialLength;

    % Radiative properties (Stefan-Boltzmann network)
    epsilon_R   = p.thermal.rotorEmissivity;
    epsilon_H   = p.thermal.housingEmissivity;
    sigmaSB     = 5.67e-8;                             % [W/(m^2·K^4)]
    ...
    for k = 1:n
        ...
        % Rotor surface area: exposed shaft + flywheel
        r_o = r_fly;
        r_i = r_shaft;

        % Shaft: cylindrical area not covered by flywheel, plus both ends
        h_shaft_exposed = max(h_shaft - h_fly, 0);
        A_shaft_cyl     = 2*pi*r_shaft*h_shaft_exposed;
        A_shaft_ends    = 2*pi*r_shaft^2;

        % Flywheel: outer cylindrical area + top/bottom annuli (cut out for shaft)
        A_fly_cyl  = 2*pi*r_o*h_fly;
        A_fly_ends = 2*pi*(r_o^2 - r_i^2);

        A_surface = A_shaft_cyl + A_shaft_ends + A_fly_cyl + A_fly_ends;

        % Explicit radiation-network denominator:
        %   Q_gen = sigma * (T_R^4 - T_H^4) / R_tot
        %   R_tot = (1-eps_R)/(eps_R*A_R) + 1/(A_R*F_RH) + (1-eps_H)/(eps_H*A_H)
        %
        % Rotor area A_R = A_surface (shaft + flywheel)
        A_R  = A_surface;

        % Housing area A_H: same construction but inflated by clearances
        %   - Flywheel: +radialFlywheel_m radial clearance, +axial_m on each end
        %   - Shaft:   +radialOther_m radial clearance
        r_clear_fly   = p.clearance.radialFlywheel_m;
        r_clear_other = p.clearance.radialOther_m;

        r_o_h   = r_fly + r_clear_fly;
        h_fly_h = h_fly + 2*axial_clear;
        r_i_h   = r_shaft + r_clear_other;

        h_shaft_exposed_h = max(h_shaft - h_fly_h, 0);
        A_shaft_cyl_h     = 2*pi*r_i_h*h_shaft_exposed_h;
        A_shaft_ends_h    = 2*pi*r_i_h^2;
        A_fly_cyl_h       = 2*pi*r_o_h*h_fly_h;
        A_fly_ends_h      = 2*pi*(r_o_h^2 - r_i_h^2);
        A_H               = A_shaft_cyl_h + A_shaft_ends_h + A_fly_cyl_h + A_fly_ends_h;

        % View factor (coaxial, close spacing)
        F_RH = 1.0;

        R_tot = (1 - epsilon_R) / (epsilon_R * A_R) + ...
                1 / (A_R * F_RH) + ...
                (1 - epsilon_H) / (epsilon_H * A_H);

        Q_gen = losses.rotor_W(k);
        T_K   = ((Q_gen * R_tot) / sigmaSB + T_housing_K^4)^(1/4);
...
function cycleResults = computeCycleEfficiency(params, P_rated_W, ...
    ...
    % Round-trip efficiency: energy output / (energy input + losses)
    % Following diagram logic: η = W_G,OUT / (W_G,IN + W_LOSSES)
    if (E_in_Wh + E_loss_Wh) > 0
        cycleResults.eta_roundtrip = E_out_Wh / (E_in_Wh + E_loss_Wh);
    else
...
function ambStep = simulateAmbStepResponse(p, pc, ambParams, m_effective)
    %SIMULATEAMBSTEPRESPONSE 1-DOF AMB step response to disturbance force.

    
    m = m_effective; % used for inertia
    Ks = ambParams.stiffnessConstant;
    Ki = ambParams.forceConstant;
    L  = ambParams.coilInductance;
    R  = ambParams.coilResistance;
    i_bias = 0;  % No bias current needed because Radial mass is zero (weight on thrust bearing)
    ...
    % Position plant: force to displacement (1-DOF point mass)
    Ks_mag = abs(Ks);
    G_pos = Ki / (m*s^2 + Ks_mag);

    % Closed-loop: disturbance force to position
    % Assumes ideal current loop: x/F_d = 1/(m*s^2 + Ks + Ki*G_cp)
    G_dist_to_x = 1 / (m*s^2 + Ks_mag + Ki*G_cp);

    % Step disturbance: 10% of rated force at t=10ms
    F_step = 0.10 * p.amb.ratedForce_N;
    t_end = 0.20;  % [s]
    t = linspace(0, t_end, 2e5);
    t_step = 0.010;  % [s]
    F_input = zeros(size(t));
    F_input(t >= t_step) = F_step;

    % Simulate step response
    [x_response, t_out] = lsim(G_dist_to_x, F_input, t);

    % Estimate current from displacement (quasi-static approximation)
    i_A = i_bias + (Ks_mag / Ki) * (-x_response);
...
function dynStiff = computeDynamicStiffness(p, pc, ambParams, m_effective, J, L_amb)
    %COMPUTEDYNAMICSTIFFNESS Dynamic stiffness vs frequency (radial & tilting).
    %   K_dyn = F_d / x. Radial: m≈0 (weight on thrust bearing). Tilting: uses J.

    m = 0 * m_effective;  % Radial mass zero (weight on thrust bearing)
    Ks = ambParams.stiffnessConstant;  % Magnitude of negative stiffness
    Ki = ambParams.forceConstant;
    ...
    % Radial: K_dyn = m*s^2 + Ks + Ki*G_cp (m=0)
    K_dyn_rad = m*s^2 + Ks + Ki * G_cp_rad;

    % Tilting: convert to rotational coordinates, two AMBs
    Ks_rot = Ks * L_amb^2;  % [Nm/rad]
    Ki_rot = Ki * L_amb;    % [Nm/A]
    K_dyn_tilt = J*s^2 + 2*(Ks_rot + Ki_rot * G_cp_tilt);
...
function runout = computeRunoutVsSoc(soc_vec, omega_vec, dynStiff, m_effective, p)
    %COMPUTERUNOUTVSSOC Runout vs SoC using ISO G2.5 and dynamic stiffness.

    % ISO 1940 G2.5: G = e * Ω = 2.5e-3 m/s
    G_grade = 2.5e-3;   % [m/s]

    % Use total rotating mass for imbalance force
    m_eff = m_effective;

    d_shaft = p.geometry.shaftAndPMDiameter_m;
    ambParams_local = ambParameters(d_shaft, p.amb.ratedForce_N);
    Ks = ambParams_local.stiffnessConstant;
    Ki = ambParams_local.forceConstant;
    
    n = numel(soc_vec);
    runout_mm = zeros(1, n);

    for k = 1:n
        omega = max(omega_vec(k), 1e-3);  % [rad/s]

        % Eccentricity from balance grade: e = G / Ω
        e_m = G_grade / omega;  % [m]

        % Imbalance force: F_d = m * e * Ω^2 = m * G * Ω
        F_d = m_eff * G_grade * omega;  % [N]

        freq_Hz = omega / (2*pi);
        K_dyn_base = interp1(dynStiff.freq_Hz, dynStiff.K_rad_N_per_m, freq_Hz, 'linear', 'extrap');

        % Use interpolated closed-loop stiffness magnitude
        K_dyn_rad = max(abs(K_dyn_base), eps);

        % Runout amplitude: r = |F_d| / |K_dyn|
        r_m = abs(F_d) / K_dyn_rad;  % [m]

        runout_mm(k) = r_m * 1e3;  % [mm]
    end
...
function runDeliverable2(params, format)
    ...
    % Plot SoC cycle with highest efficiency design
    plotHighestEfficiencyCycle(designResults, params, format);
...
function geom = sizeFlywheelFromRequirements(t_mag, omega_max_candidate, ...
    ...
    % Size components based on candidate max speed
    % Shaft and flywheel are sized to meet their tip speed limits at this speed
    r_shaft = p.speed.maxTsShaft_mPerSec / omega_max_candidate;
    d_shaft = 2 * r_shaft;
    r_fly = p.speed.maxTsFly_mPerSec / omega_max_candidate;
    
    % Check PM tip speed limit (depends on sized shaft radius)
    r_pm = r_shaft + t_mag;
    omega_max_pm = p.speed.maxTsPM_mPerSec / r_pm;
    
    % Actual max speed is limited by PM tip speed if its more restrictive
    % Shaft and flywheel are already sized correctly for the candidate speed
    omega_max = min(omega_max_candidate, omega_max_pm);
    ...
    if h_fly <= 0 || h_fly > 7  % Max 7m seems reasonable?
    ...
    % First, estimate shaft length without AMB to get initial mass estimate
    axial_clear = p.clearance.axial_m;
    % Use a rough estimate for AMB length (will refine after we know force rating)
    amb_length_estimate = 0.1; % [m] rough estimate
    h_shaft_estimate = (5 * axial_clear) + h_fly + h_motor + 2 * amb_length_estimate;
    
    % Compute initial mass estimate
    vol_shaft_estimate = pi * r_shaft^2 * h_shaft_estimate;
    vol_fly = pi * (r_fly^2 - r_shaft^2) * h_fly;
    m_shaft_estimate = rho_steel * vol_shaft_estimate;
    m_fly = rho_fly * vol_fly;
    m_rotating_estimate = m_shaft_estimate + m_fly;
    
    % Calculate AMB rated force: each bearing rated for 2x rotating group weight
    g = 9.81; % [m/s^2] gravitational acceleration
    weight_N = m_rotating_estimate * g; % [N] rotating group weight
    amb_ratedForce_N = 2 * weight_N; % [N] each bearing rated for 2x weight
    
    % Get AMB parameters based on calculated force rating
    ambParams = ambParameters(d_shaft, amb_ratedForce_N);
    
    % Recalculate shaft length with actual AMB length
    h_shaft = (5 * axial_clear) + h_fly + h_motor + 2 * ambParams.axialLength;
...
function [isFeasible, cycleEfficiency] = checkCycleFeasibility( ...
    ...
    % Initial KE at 50% SoC
    soc0 = 0.5; % 50%
    KE_initial = KE_min + soc0 * (KE_max - KE_min);
    KE = KE_initial;
    ...
    for k = 1:numel(t_s)-1
        ...
        maxSoC = max(maxSoC, soc);
        
        % Early exit if infeasibility detected
        if soc < 0 || soc > 100
            % Infeasible - KE went outside valid range
            % Still update final KE for completeness
            KE_final = KE;
            isFeasible = false;
            % Set efficiency to 0 for infeasible designs
            cycleEfficiency = 0;
            return;
        end
    ...
    % Check feasibility: SoC must stay between 0% and 100% throughout
    % (which corresponds to KE staying between KE_min and KE_max)
    isFeasible = (minSoC >= 0) && (maxSoC <= 100);
    
    % Compute cycle efficiency: energy output / (energy input + losses)
    % Following diagram logic: η = W_G,OUT / (W_G,IN + W_LOSSES)
    if (E_in_Wh + E_loss_Wh) > 0
        cycleEfficiency = E_out_Wh / (E_in_Wh + E_loss_Wh);
    else
        cycleEfficiency = 0;
    end
...
function plotHighestEfficiencyCycle(designResults, params, format)
    %PLOTHIGHESTEFFICIENCYCYCLE Plot SoC cycle for design with highest efficiency.
    %   Finds the feasible design with the highest cycle efficiency and plots
    %   its SoC trajectory over team_2_cycle along with the power profile.
    ...
    if isempty(feasibleResults)
        fprintf('Warning: No feasible designs found. Cannot plot highest efficiency cycle.\n');
        return;
    end

    % Find design with highest efficiency
    efficiency = [feasibleResults.cycleEfficiency];
    [maxEfficiency, maxEffIdx] = max(efficiency);
    highestEffDesign = feasibleResults(maxEffIdx);
    
    fprintf('\n=== Highest Efficiency Design Cycle Plot ===\n');
    fprintf('Design with highest efficiency: %.2f%%\n', maxEfficiency * 100);
    fprintf('  Magnet thickness: %.2f mm\n', highestEffDesign.t_mag * 1e3);
    fprintf('  Maximum speed: %.0f rpm\n', highestEffDesign.omega_max * 60/(2*pi));
    fprintf('\n');

    % Extract geometry and parameters
    geom = highestEffDesign.geometry;
    J = geom.J;
    KE_min = geom.KE_min;
    KE_max = geom.KE_max;
    
    % Compute losses vs SoC for this design
    p = params.deliverable1;
    soc_vec = linspace(0, 100, 100);
    [losses, ~, omega_vec] = computeLossesAndTemperatureVsSoc( ...
        soc_vec, J, KE_min, KE_max, highestEffDesign.P_rated, ...
        geom.T_rated, geom.r_fly, geom.h_fly, geom.d_shaft, ...
        highestEffDesign.t_mag, p);

    % Plot SoC cycle with power profile
    plotSocCycleWithPowerProfile(J, KE_min, KE_max, soc_vec, omega_vec, ...
        losses, @team_2_cycle, format, maxEfficiency);

end

function plotSocCycleWithPowerProfile(J, KE_min, KE_max, soc_vec_grid, ...
    omega_vec_grid, losses, cycleFunc, format, efficiency)
    %PLOTSOCYCLEWITHPOWERPROFILE Plot SoC vs time with power profile overlay.
    %   Plots the SoC trajectory over the cycle and the power profile on
    %   the same axes (dual y-axis).
    %
    %   Inputs:
    %     J: moment of inertia [kg·m²]
    %     KE_min: minimum kinetic energy [J]
    %     KE_max: maximum kinetic energy [J]
    %     soc_vec_grid: SoC grid for loss interpolation [%]
    %     omega_vec_grid: angular speed grid [rad/s]
    %     losses: structure with rotor_W and stator_W fields
    %     cycleFunc: function handle for cycle (e.g., @team_2_cycle)
    %     format: plotting format structure
    %     efficiency: cycle efficiency for title
    ...
    % Create figure with doubled width and stacked subplots
    figure('Position', [200, 200, 2 * format.figWidth, format.figHeight]);
    
    % Top subplot: SoC
    subplot(2, 1, 1);
    plot(t_s/3600, soc_vec, 'Color', format.color.primary, ...
        'LineWidth', format.lineWidth, 'DisplayName', 'State of Charge');
    ylabel('State of Charge [%]', 'FontName', format.fontName, ...
        'FontSize', format.fontSize);
    ylim([0 100]);
    grid on;
    title(sprintf('SoC Cycle with Highest Efficiency Design (\\eta = %.1f%%)', ...
        efficiency * 100), 'FontName', format.fontName, 'FontSize', format.fontSize);
    legend('Location', 'best', 'FontName', format.fontName, ...
        'FontSize', format.fontSize);
    set(gca, 'FontName', format.fontName, 'FontSize', format.fontSize);
    
    % Bottom subplot: Power profile with shaded area
    subplot(2, 1, 2);
    P_grid_kW = P_grid_W / 1e3;
    t_hours = t_s / 3600;
    % Use area function to shade between power profile and zero
    area(t_hours, P_grid_kW, 'FaceColor', format.color.secondary, ...
        'FaceAlpha', 0.3, 'EdgeColor', format.color.secondary, ...
        'LineWidth', format.lineWidth, 'DisplayName', 'Power Profile');
    ylabel('Power [kW]', 'FontName', format.fontName, ...
        'FontSize', format.fontSize);
    xlabel('Time [hours]', 'FontName', format.fontName, ...
        'FontSize', format.fontSize);
    grid on;
    legend('Location', 'best', 'FontName', format.fontName, ...
        'FontSize', format.fontSize);
